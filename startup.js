...
function checkOrientation() {
	//when changing from potrait to landscape change to the rigth width
	...
	for (var i = 0; i < _pages.length; ++i) {
		if (Array.isArray(_pages[i])) {
			...
			if (pageConfig.type == 'generic') {
				...
				likertSingleStimulusPageManager.createPages(
					_pageManager,
					pageTemplateRenderer,
					pageConfig,
					audioContext,
				...
				var likertMultiStimulusPage = new LikertMultiStimulusPage(
					pageManager,
					pageTemplateRenderer,
					pageConfig,
					audioContext,
...
for (var i = 0; i < $('body').children().length; i++) {
	if (
		$('body').children().eq(i).attr('id') != 'popupErrors' &&
		$('body').children().eq(i).attr('id') != 'popupDialog'
	) {
...
function startup(config) {
	...
	if (pageManager !== null) {
		// clear everything for new experiment
