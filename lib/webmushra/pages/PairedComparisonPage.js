		...
		function (_event) {
			if (_event.name == 'stopTriggered') {
				...
				if ($('#buttonConditions0').attr('active') == 'true') {
					$.mobile.activePage
						.find('#buttonConditions0') //remove color from Reference
						.removeClass('ui-btn-b')
						.addClass('ui-btn-a')
						.attr('data-theme', 'a')
				...
				if ($('#buttonConditions1').attr('active') == 'true') {
					$.mobile.activePage
						.find('#buttonConditions1') //remove color from Reference
						.removeClass('ui-btn-b')
						.addClass('ui-btn-a')
						.attr('data-theme', 'a')
  ...
  '
	...
	var buttonPlayA = $(
		"<td><button data-theme='a' id='buttonConditions0' data-role='button' class='audioControlElement' onclick='" +
			this.pageManager.getPageVariableName(this) +
			".btnCallbackA()' style='margin : 0 auto;'>" +
			this.pageManager.getLocalizer().getFragment(this.language, 'playButton') +
			'</button></td>'
	)
	...
	var buttonPlayB = $(
		"<td><button data-theme='a' id='buttonConditions1' data-role='button' class='audioControlElement' onclick='" +
			this.pageManager.getPageVariableName(this) +
			".btnCallbackB()' style='margin : 0 auto;'>" +
			this.pageManager.getLocalizer().getFragment(this.language, 'playButton') +
			'</button></td>'
	)
	...
	var trQuestion = $(
		"<tr><td  colspan='3'><br/><br/>" +
			this.pageManager.getLocalizer().getFragment(this.language, 'quest') +
			'</td></tr>'
	)
	...
	var radioChoice = $(
		"<div id='radio-choice' data-role='controlgroup' data-type='horizontal'>\
	...
	if (this.pageConfig.unforced) {
		radioChoice.append(
			$(
				"<input type='radio' name='radio-choice' id='radio-choice-n' value='n'><label for='radio-choice-n'>" +
					this.pageConfig.unforced +
					'</label>'
			)
		)
	}

	radioChoice.find("input[type='radio']").bind(
		'change',
		function () {
...
PairedComparisonPage.prototype.btnCallbackA = function () {
	...
	if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
		...
		this.cleanButtons()
		$.mobile.activePage
			.find('#buttonConditions0') //add color to reference
			.removeClass('ui-btn-a')
			.addClass('ui-btn-b')
			.attr('data-theme', 'b')
...
PairedComparisonPage.prototype.btnCallbackB = function () {
	...
	if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
		...
		this.cleanButtons()
		$.mobile.activePage
			.find('#buttonConditions1') //add color to reference
			.removeClass('ui-btn-a')
			.addClass('ui-btn-b')
			.attr('data-theme', 'b')
