	...
	for (var i = 0; i < this.pageConfig.views.length; ++i) {
		...
		this.renderDivs[i].onmousedown = function (event) {
			...
			if (this.page.activeResponse !== null) {
				...
				var positions = this.scene.transformRelativeCoordinates(
					relPosX,
					relPosY,
					centerPosition[0],
					centerPosition[1],
...
SpatialPage.prototype.render = function (_id) {
	...
	for (var i = 0; i < this.pageConfig.responses.length; ++i) {
		...
		button = $(
			"<button id='" +
				respConfig.name +
				"' data-inline='true' style='background-color:#" +
				((1 << 24) + respConfig.color).toString(16).substr(1) +
				'; background-image:radial-gradient(#ffffff 0%, ' +
				color +
				" 100%)'>" +
				respConfig.label +
				'</button>'
