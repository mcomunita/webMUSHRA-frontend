...
function LikertSingleStimulusPage(
	_pageManager,
...
LikertSingleStimulusPage.prototype.init = function (_callbackError) {
	...
	if (this.pageConfig.mustPlayback) {
		this.filePlayer.genericAudioControl.addEventListener(
			function (_event) {
				if (_event.name == this.pageConfig.mustPlayback) {
					this.likerts.map(function (l) {
						l.enable()
					})
				}
			}.bind(this)
...
LikertSingleStimulusPage.prototype.render = function (_parent) {
	...
	if (this.pageConfig.showWaveform === true) {
		...
		this.waveformVisualizer = new WaveformVisualizer(
			...
			this.pageConfig.showWaveform,
			false,
			this.filePlayer.genericAudioControl
		)
...
LikertSingleStimulusPage.prototype.load = function () {
	...
	if (this.results.length > 0) {
		for (var i = 0; i < this.likerts.length; ++i) {
			$("input[name='" + this.likerts[i].prefix + "_response'][value='" + this.results[i] + "']").attr(
				'checked',
				'checked'
			)
			$("input[name='" + this.likerts[i].prefix + "_response'][value='" + this.results[i] + "']").checkboxradio(
				'refresh'
			)
