...
FinishPage.prototype.storeParticipantData = function () {
	...
	for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
		...
		if ($('#' + element.name).val()) {
			...
			this.session.participant.response[this.session.participant.response.length] = $(
...
FinishPage.prototype.render = function (_parent) {
	...
	for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
		...
		if (element.type === 'text') {
			...
			var td = $('<td></td>')
			table.append($('<tr></tr>').append($('<td><strong>' + element.label + '</strong></td>'), td))
			this.likert.render(td)
		} else if (element.type === 'long_text') {
			table.append(
				$(
					"<tr><td id='labeltd' style='vertical-align:top; padding-top:" +
			...
			function () {
				...
				for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
					...
					if (element.type === 'text') {
						...
						if (
							(this.likert && $("input[name='" + element.name + "__response']:checked").val()) ||
