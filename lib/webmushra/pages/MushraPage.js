...
MushraPage.prototype.render = function (_parent) {
  ...
  '
	...
	var tdConditionNamesReference = $(
		'<td>' + this.pageManager.getLocalizer().getFragment(this.language, 'reference') + '</td>'
	)
...
MushraPage.prototype.renderCanvas = function (_parentId) {
	...
	var xAbsTableOffset =
		-$('#mushra_items').offset().left - ($('#mushra_canvas').offset().left - $('#mushra_items').offset().left)
	var xDrawingBeforeScales = $('.scales').first().prev().children().eq(0).offset().left + xAbsTableOffset
	var xDrawingEnd =
		$('.scales').last().offset().left - $('#mushra_items').offset().left + $('.scales').last().width() / 2
