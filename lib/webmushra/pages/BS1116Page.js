...
of the Software License for the webMUSHRA.js Software. Said terms and 
conditions have been made available to You prior to Your download of this 
source code. By downloading this source code You agree to be bound by the 
above mentionend terms and conditions, which can also be found here: 
https://www.audiolabs-erlangen.de/resources/webMUSHRA. Any unauthorised use 
of this source code may result in severe civil and criminal penalties, 
        ...
        function (_event) {
            if (_event.name == 'stopTriggered') {
                ...
                if ($('#buttonConditions0').attr('active') == 'true') {
                    $.mobile.activePage
                        .find('#buttonConditions0') //remove color from Condition0
                        .removeClass('ui-btn-b')
                        .addClass('ui-btn-a')
                        .attr('data-theme', 'a')
                ...
                if ($('#buttonConditions1').attr('active') == 'true') {
                    $.mobile.activePage
                        .find('#buttonConditions1') //remove color from Condition1
                        .removeClass('ui-btn-b')
                        .addClass('ui-btn-a')
                        .attr('data-theme', 'a')
...
BS1116Page.prototype.render = function (_parent) {
  ...
  '
    ...
    var buttonPlayReference = $(
        "<td id='tdPlayButton' ><button data-theme='a' id='buttonReference' data-role='button' class='audioControlElement' onclick='" +
            this.pageManager.getPageVariableName(this) +
    ...
    var buttonPlayC = $(
        "<td><button data-theme='a' id='buttonConditions1' data-role='button' class='audioControlElement' onclick='" +
            this.pageManager.getPageVariableName(this) +
            ".btnCallbackC()' >" +
            this.pageManager.getLocalizer().getFragment(this.language, 'playButton') +
            '</button></td>'
    ...
    for (var i = 0; i < 2; ++i) {
      ...
      <span><input type='range'  class='scales' id='rating" +
                i +
                "' value='5' min='1' max='5' step='0.01' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/></span> \
    </td>"
        )
...
BS1116Page.prototype.renderCanvas = function (_parentId) {
    ...
    var YfirstLine =
        $('.scales').parent().get(0).offsetTop + parseInt($('.scales').css('borderTopWidth'), 10) + YfreeCanvasSpace
    var prevScalesHeight = $('.scales').prev().height()
    var xDrawingStart =
        $('#spaceForScale').offset().left -
        $('#spaceForScale').parent().offset().left +
        canvasContext.measureText('Perceptible, but not annoying    4').width * 1.5
    var xAbsTableOffset =
        -$('#tableBS1116').offset().left - ($('#BS116canvas').offset().left - $('#tableBS1116').offset().left)
    var xDrawingBeforeScales = $('.scales').first().prev().children().eq(0).offset().left + xAbsTableOffset
    var xDrawingEnd =
        $('.scales').last().offset().left - $('#tableBS1116').offset().left + $('.scales').last().width() / 2
    ...
    var xLetters =
        $('#refCanvas').width() +
        ($('#spaceForScale').width() + canvasContext.measureText('Perceptible, but not annoying    4 ').width) / 2.0
    ...
    canvasContext.fillText(
        this.pageManager.getLocalizer().getFragment(this.language, 'imperceptible') + '    5',
        xTextScoreRanges,
        YfirstLine
    )
    canvasContext.fillText(
        this.pageManager.getLocalizer().getFragment(this.language, 'perceptible') + '    4',
        xTextScoreRanges,
        prevScalesHeight * 0.25 + YfirstLine
    )
    canvasContext.fillText(
        this.pageManager.getLocalizer().getFragment(this.language, 'slightly') + '    3',
        xTextScoreRanges,
        prevScalesHeight * 0.5 + YfirstLine
    )
    canvasContext.fillText(
        this.pageManager.getLocalizer().getFragment(this.language, 'annoying') + '    2',
        xTextScoreRanges,
        prevScalesHeight * 0.75 + YfirstLine
    )
    canvasContext.fillText(
        this.pageManager.getLocalizer().getFragment(this.language, 'very') + '    1',
        xTextScoreRanges,
        prevScalesHeight + YfirstLine
    )
}
...
BS1116Page.prototype.cleanButtons = function () {
    ...
    if ($('#buttonConditions0').attr('active') == 'true') {
        $.mobile.activePage
            .find('#buttonConditions0') //remove color from Conditions0
            .removeClass('ui-btn-b')
            .addClass('ui-btn-a')
            .attr('data-theme', 'a')
        $('#rating0').slider('disable')
    ...
    if ($('#buttonConditions1').attr('active') == 'true') {
        $.mobile.activePage
            .find('#buttonConditions1') //remove color from Conditions1
            .removeClass('ui-btn-b')
            .addClass('ui-btn-a')
            .attr('data-theme', 'a')
        $('#rating1').slider('disable')
...
BS1116Page.prototype.btnCallbackB = function () {
    ...
    if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
        ...
        this.cleanButtons()
        $.mobile.activePage
            .find('#buttonConditions0') //add color to Condition0
            .removeClass('ui-btn-a')
            .addClass('ui-btn-b')
            .attr('data-theme', 'b')
...
BS1116Page.prototype.btnCallbackC = function () {
    ...
    if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
        ...
        this.cleanButtons()
        $.mobile.activePage
            .find('#buttonConditions1') //add color to Conditions1
            .removeClass('ui-btn-a')
            .addClass('ui-btn-b')
            .attr('data-theme', 'b')
