...
Scene.prototype.transformRelativeCoordinates = function (_relX, _relY, _activeObjX, _activeObjY, _activeObjZ) {
	...
	if (this.wmCamera.type == 1) {
		...
		var cameraDir = new THREE.Vector3(
			-this.wmCamera.getCameraX(),
			-this.wmCamera.getCameraY(),
			-this.wmCamera.getCameraZ()
		)
		...
		quaternion_y.setFromUnitVectors(
			new THREE.Vector3(0, 0, -1),
			new THREE.Vector3(cameraDir_n.x, 0, cameraDir_n.z).normalize()
		)
		...
		newPos.multiplyScalar(
			Math.sqrt(
				Math.pow(cameraDir.length() + shift, 2) +
					Math.pow((newPos_vector.length() * (cameraDir.length() + shift)) / cameraDir.length(), 2)
			)
		)
