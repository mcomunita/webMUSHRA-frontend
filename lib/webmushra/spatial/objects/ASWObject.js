...
ASWObject.prototype.updateControlPolygon = function () {
	...
	this.geometryLine1.vertices.push(
		new THREE.Vector3(this.controlPoints[0][0], this.controlPoints[0][1], this.controlPoints[0][2])
	)
	this.geometryLine1.vertices.push(
		new THREE.Vector3(this.controlPoints[1][0], this.controlPoints[1][1], this.controlPoints[1][2])
	...
	if (this.height != 0 && this.depth != 0) {
		this.midpointOnCurve[0] =
			0.125 * this.controlPoints[0][0] +
			0.375 * this.controlPoints[1][0] +
			0.375 * this.controlPoints[2][0] +
			0.125 * this.controlPoints[3][0]
		this.midpointOnCurve[1] =
			0.125 * this.controlPoints[0][1] +
			0.375 * this.controlPoints[1][1] +
			0.375 * this.controlPoints[2][1] +
			0.125 * this.controlPoints[3][1]
		this.midpointOnCurve[2] =
			0.125 * this.controlPoints[0][2] +
			0.375 * this.controlPoints[1][2] +
			0.375 * this.controlPoints[2][2] +
			0.125 * this.controlPoints[3][2]
		this.derivative[0] =
			3 *
			(this.controlPoints[0][0] + this.controlPoints[1][0] - this.controlPoints[2][0] - this.controlPoints[3][0])
		this.derivative[1] =
			3 *
			(this.controlPoints[0][1] + this.controlPoints[1][1] - this.controlPoints[2][1] - this.controlPoints[3][1])
		this.derivative[2] =
			3 *
			(this.controlPoints[0][2] + this.controlPoints[1][2] - this.controlPoints[2][2] - this.controlPoints[3][2])
		...
		this.geometryLine2.vertices.push(
			new THREE.Vector3(this.controlPoints[4][0], this.controlPoints[4][1], this.controlPoints[4][2])
		)
		this.geometryLine2.vertices.push(
			new THREE.Vector3(this.midpointOnCurve[0], this.midpointOnCurve[1], this.midpointOnCurve[2])
		)
		this.geometryLine2.vertices.push(
			new THREE.Vector3(this.controlPoints[5][0], this.controlPoints[5][1], this.controlPoints[5][2])
		)
...
ASWObject.prototype.eventOnMouseDown = function (_event, _x, _y, _z) {
	...
	if (_x == null) {
		...
		for (var i = 0; i < this.controlPoints.length; ++i) {
			if (
				this.distance(
					new THREE.Vector3(this.controlPoints[i][0], _y, _z),
					new THREE.Vector3(this.controlPoints[i][0], this.controlPoints[i][1], this.controlPoints[i][2])
				) < threshold
			) {
				this.activeObj = i
			}
		...
		for (var i = 0; i < this.controlPoints.length; ++i) {
			if (
				this.distance(
					new THREE.Vector3(_x, this.controlPoints[i][1], _z),
					new THREE.Vector3(this.controlPoints[i][0], this.controlPoints[i][1], this.controlPoints[i][2])
				) < threshold
			) {
				this.activeObj = i
	...
	if (_z == null) {
		...
		for (var i = 0; i < this.controlPoints.length; ++i) {
			if (
				this.distance(
					new THREE.Vector3(_x, _y, this.controlPoints[i][2]),
					new THREE.Vector3(this.controlPoints[i][0], this.controlPoints[i][1], this.controlPoints[i][2])
				) < threshold
			) {
				this.activeObj = i
...
ASWObject.prototype.eventOnMouseMove = function (_event, _x, _y, _z) {
	if (this.activeObj != null) {
		if (this.activeObj == 4 && _y != null) {
			...
			this.depth = Math.sqrt(
				Math.pow(_x - this.midpointOnCurve[0], 2) + Math.pow(_z - this.midpointOnCurve[2], 2)
			)
		} else {
			if (_x != null) {
