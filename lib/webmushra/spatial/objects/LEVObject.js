...
function LEVObject(_objConfig) {
	...
	this.point3 = new THREE.Vector3(
		_objConfig.position[0],
		_objConfig.size[1] + _objConfig.position[1],
		_objConfig.position[2]
	)
...
LEVObject.prototype.eventOnMouseDown = function (_event, _x, _y, _z) {
	...
	if (_x == null) {
		if (
			this.distance(
				new THREE.Vector3(this.dragNdrop0.position.x, _y, _z),
				new THREE.Vector3(this.dragNdrop0.position.x, this.dragNdrop0.position.y, this.dragNdrop0.position.z)
			) < threshold
		) {
			this.activeObj = 0
		} else if (
			this.distance(
				new THREE.Vector3(this.dragNdrop1.position.x, _y, _z),
				new THREE.Vector3(this.dragNdrop1.position.x, this.dragNdrop1.position.y, this.dragNdrop1.position.z)
			) < threshold
		) {
			this.activeObj = 1
		} else if (
			this.distance(
				new THREE.Vector3(this.dragNdrop2.position.x, _y, _z),
				new THREE.Vector3(this.dragNdrop2.position.x, this.dragNdrop2.position.y, this.dragNdrop2.position.z)
			) < threshold
		) {
			this.activeObj = 2
		} else if (
			this.distance(
				new THREE.Vector3(this.dragNdrop3.position.x, _y, _z),
				new THREE.Vector3(this.dragNdrop3.position.x, this.dragNdrop3.position.y, this.dragNdrop3.position.z)
			) < threshold
		) {
			this.activeObj = 3
		} else {
		...
		if (
			this.distance(
				new THREE.Vector3(_x, this.dragNdrop0.position.y, _z),
				new THREE.Vector3(this.dragNdrop0.position.x, this.dragNdrop0.position.y, this.dragNdrop0.position.z)
			) < threshold
		) {
			this.activeObj = 0
			...
			this.distance(
				new THREE.Vector3(_x, this.dragNdrop1.position.y, _z),
				new THREE.Vector3(this.dragNdrop1.position.x, this.dragNdrop1.position.y, this.dragNdrop1.position.z)
			) < threshold
		) {
			this.activeObj = 1
			...
			this.distance(
				new THREE.Vector3(_x, this.dragNdrop2.position.y, _z),
				new THREE.Vector3(this.dragNdrop2.position.x, this.dragNdrop2.position.y, this.dragNdrop2.position.z)
			) < threshold
		) {
			this.activeObj = 2
			...
			this.distance(
				new THREE.Vector3(_x, this.dragNdrop3.position.y, _z),
				new THREE.Vector3(this.dragNdrop3.position.x, this.dragNdrop3.position.y, this.dragNdrop3.position.z)
			) < threshold
		) {
			this.activeObj = 3
	...
	if (_z == null) {
		if (
			this.distance(
				new THREE.Vector3(_x, _y, this.dragNdrop0.position.z),
				new THREE.Vector3(this.dragNdrop0.position.x, this.dragNdrop0.position.y, this.dragNdrop0.position.z)
			) < threshold
		) {
			this.activeObj = 0
			...
			this.distance(
				new THREE.Vector3(_x, _y, this.dragNdrop1.position.z),
				new THREE.Vector3(this.dragNdrop1.position.x, this.dragNdrop1.position.y, this.dragNdrop1.position.z)
			) < threshold
		) {
			this.activeObj = 1
			...
			this.distance(
				new THREE.Vector3(_x, _y, this.dragNdrop2.position.z),
				new THREE.Vector3(this.dragNdrop2.position.x, this.dragNdrop2.position.y, this.dragNdrop2.position.z)
			) < threshold
		) {
			this.activeObj = 2
			...
			this.distance(
				new THREE.Vector3(_x, _y, this.dragNdrop3.position.z),
				new THREE.Vector3(this.dragNdrop3.position.x, this.dragNdrop3.position.y, this.dragNdrop3.position.z)
			) < threshold
		) {
			this.activeObj = 3
...
LEVObject.prototype.setPosition1 = function (_x, _y, _z) {
	...
	var dist1 = Math.sqrt(
		(this.point1.x - this.point0.x) * (this.point1.x - this.point0.x) +
			(this.point1.z - this.point0.z) * (this.point1.z - this.point0.z)
	)
	var dist2 = Math.sqrt(
		(this.point2.x - this.point0.x) * (this.point2.x - this.point0.x) +
			(this.point2.z - this.point0.z) * (this.point2.z - this.point0.z)
	)
