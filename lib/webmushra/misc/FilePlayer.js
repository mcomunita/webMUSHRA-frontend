...
function FilePlayer(_audioContext, _bufferSize, _stimuli, _errorHandler, _language, _localizer, _hideProgressBar) {
	...
	this.genericAudioControl = new GenericAudioControl(
		this.audioContext,
		this.bufferSize,
		this.stimuli,
		this.errorHandler
	...
	this.genericAudioControl.addEventListener(
		function (_event) {
			if (_event.name == 'processUpdate') {
				$('#FilePlayer_progressbar_' + _event.index).progressbar(
					'option',
					'value',
					(_event.currentSample / (_event.numSamples - 1)) * 100
				)
			}
		}.bind(this)
	)
...
FilePlayer.prototype.init = function () {
	...
	if (this.hideProgressBar != true) {
		for (var i = 0; i < this.stimuli.length; ++i) {
			TolitoProgressBar(this.progressBars[i].attr('id'))
				.setOuterTheme('a')
				.setInnerTheme('b')
				.isMini(true)
				.setMax(100)
...
FilePlayer.prototype.renderElement = function (_parent, _i) {
	var buttonPlay = $(
		'<button data-role="button" id="playCondition' +
			_i +
			'" name="' +
			_i +
			'">' +
			this.localizer.getFragment(this.language, 'playButton') +
			'</button>'
	)
	buttonPlay.bind(
		'click',
	...
	var buttonPause = $(
		'<button data-role="button" id="pause">' +
			this.localizer.getFragment(this.language, 'pauseButton') +
			'</button>'
	)
	buttonPause.bind(
		'click',
